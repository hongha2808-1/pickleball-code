...
function get_active_sliders() {
    try {
        // Lấy slides từ bảng tbl_slides của admin
        return db_fetch_array("SELECT * FROM `tbl_slides` WHERE `status` = 1 ORDER BY `slide_order` ASC, `id` DESC");
    } catch (Exception $e) {
        // Nếu có lỗi, trả về mảng rỗng để tránh làm gián đoạn trang
        return array();
    }
}
