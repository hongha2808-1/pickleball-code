            ...
            <div class="section" id="detail-page">
                <div class="section-detail">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <input type="text" name="product_title" id="product_title" value="<?php echo isset($_POST['product_title']) ? $_POST['product_title'] : ''; ?>">
                        <?php echo isset($error['product_title']) ? "<p class='error'>{$error['product_title']}</p>" : ""; ?>
                        
                        <label for="price_new">Giá mới</label>
                        <input type="text" name="price_new" id="price_new" value="<?php echo isset($_POST['price_new']) ? $_POST['price_new'] : ''; ?>">
                        <?php echo isset($error['price_new']) ? "<p class='error'>{$error['price_new']}</p>" : ""; ?>
                        
                        <label for="price_old">Giá cũ (để trống nếu không có)</label>
                        <input type="text" name="price_old" id="price_old" value="<?php echo isset($_POST['price_old']) ? $_POST['price_old'] : ''; ?>">
                        <?php echo isset($error['price_old']) ? "<p class='error'>{$error['price_old']}</p>" : ""; ?>
                        
                        <label for="product_desc">Mô tả ngắn</label>
                        <textarea name="product_desc" id="product_desc"><?php echo isset($_POST['product_desc']) ? $_POST['product_desc'] : ''; ?></textarea>
                        <?php echo isset($error['product_desc']) ? "<p class='error'>{$error['product_desc']}</p>" : ""; ?>
                        
                        <label for="product_content">Chi tiết</label>
                        <textarea name="product_content" id="product_content" class="ckeditor"><?php echo isset($_POST['product_content']) ? $_POST['product_content'] : ''; ?></textarea>
                        ...
                        <div id="uploadFile" style="margin-bottom: 15px;">
                            ...
                            <?php if(isset($uploaded_image) && !empty($uploaded_image)): ?>
                                <div id="current-image" style="margin-top: 15px;">
                                    <p style="font-weight: bold; margin-bottom: 5px;">Ảnh đã tải lên:</p>
                        ...
                        <select name="cat_id">
                            ...
                            <?php if(!empty($categories)): ?>
                                ...
                                <option value="<?php echo $category['id']; ?>" <?php echo isset($_POST['cat_id']) && $_POST['cat_id'] == $category['id'] ? 'selected' : ''; ?>><?php echo $category['cat_title']; ?></option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
